...
def warehouse_list():
    ...
    try:
        with conn.cursor() as cur:
            if name:
                cur.execute(
                    'SELECT id, name, address, remark FROM biz_warehouse'
...
def warehouse_add():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                'INSERT INTO biz_warehouse (name, address, remark) VALUES (%s, %s, %s) RETURNING id',
                (name, address, remark)
            )
...
def warehouse_update():
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                'UPDATE biz_warehouse SET name=%s, address=%s, remark=%s WHERE id=%s',
                (name, address, remark, wh_id)
            )
        conn.commit()
    except Exception as e:
        conn.rollback()
