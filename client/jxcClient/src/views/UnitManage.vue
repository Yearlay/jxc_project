  ...
  <div class="page-container">
    ...
    >
      <a-form :model="form" :rules="rules" ref="formRef" layout="vertical">
        <a-form-item label="计量单位名称" name="name">
          <a-input v-model:value="form.name" placeholder="请输入计量单位名称" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
async function loadList() {
  loading.value = true
  try {
    const res = await getUnitList(searchName.value)
    list.value = res.data || []
  } finally {
    loading.value = false
  }
...
async function handleSubmit() {
  ...
  try {
    if (editingId.value) {
      await updateUnit({ id: editingId.value, ...form.value })
...
function confirmDelete(record) {
  Modal.confirm({
    title: '确定删除该计量单位？',
    content: `将删除计量单位"${record.name}"，删除后不可恢复。`,
    okText: '确定删除',
    okType: 'danger',
    cancelText: '取消',
